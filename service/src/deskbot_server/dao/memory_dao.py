...
class MemoryDao(metaclass=SingletonMeta):
    def load_entries(self, *, device_id: str | None = None) -> list[dict[str, Any]]:
        return _store.load_memory_entries(device_id=device_id)

    def save_entries(self, entries: list[dict[str, Any]], *, device_id: str | None = None) -> None:
        _store.save_memory_entries(entries, device_id=device_id)

    def add(self, text: str, *, device_id: str | None = None) -> dict[str, Any]:
        return _store.add_memory(text, device_id=device_id)

    def delete(self, entry_id: str, *, device_id: str | None = None) -> bool:
        return _store.delete_memory(entry_id, device_id=device_id)

    def list_for_device(self, device_id: str | None = None, *, limit: int | None = None) -> list[dict[str, Any]]:
        if limit is None:
